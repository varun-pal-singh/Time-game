...
function ScoreBoard() {
    ...
    const previewTime = Math.max(2, 5 - gameLevel * 0.5);
    ...
    return (
        ...
        <Box container sx={{ display: "flex", flexDirection: 'row', justifyContent: "center", height: "130px" }}>
            ...
            <Slide direction="up" in={showPreviewCounter} mountOnEnter unmountOnExit>
               <div> <ProgressTimer  value={(previewSeconds /(previewTime) * 100)}  time={previewSeconds} title={"PREVIEW TIME"}  customcolor={"rgb(23, 105, 170)"} /> </div>
                </Slide>

                <Slide direction="down" in={!showPreviewCounter} mountOnEnter unmountOnExit>
               <div> <ProgressTimer  value={(selectionSeconds / (previewTime) * 100)}  time={selectionSeconds} title={" SELECTION TIME"} customcolor={"orangered"} /> </div>
                </Slide>
            </Box>
